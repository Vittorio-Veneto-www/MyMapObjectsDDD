...
namespace MyMapObjects
{
    public partial class moMapControl : UserControl
    {
        ...
        private Bitmap mBufferMap2 = new Bitmap(10, 10);    // 缓冲位图2，含要素、标记及跟踪图形
        private Bitmap mBufferMap3 = new Bitmap(10, 10);    // 缓冲位图3
        // 在屏幕上绘制2时，先将2绘制到3上，再将3绘制到控件上，以避免清除边界带来的闪烁
        private moSimpleMarkerSymbol mSelectedPointSymbol;  // 选择的点的符号
        private moSimpleLineSymbol mSelectedLineSymbol; // 选择的线的符号
        private moSimpleFillSymbol mSelectedFillSymbol; // 选择的面的符号
        private moShapeFlashControler mFlashControler = new moShapeFlashControler();    //闪烁控制器
        private moSimpleMarkerSymbol mFlashPointSymbol; // 闪烁的点的符号
        private moSimpleLineSymbol mFlashLineSymbol;    // 闪烁的线的符号
        private moSimpleFillSymbol mFlashFillSymbol;    // 闪烁的面的符号
        ...
        public void ZoomToRatio(moPoint center, double ratio)
        {
            mMapDrawingReference.ZoomByCenter(center, mMapDrawingReference.MapScale / ratio);
            this.UseWaitCursor = true;
            DrawBufferMap1();
            DrawBufferMap2();
            this.UseWaitCursor = false;
        ...
        public void SelectByBox(moRectangle selectingBox, double tolerance,Int32 selectMethod)
        {
            ...
            for (Int32 i = 0; i <= sLayerCount - 1; i++)
            {
                ...
                if(sLayer.Visible==true && sLayer.Selectable==true)
                {
                    ...
                    sLayer.ExcuteSelect(sFeatures, selectMethod);
                }
                else
                {
