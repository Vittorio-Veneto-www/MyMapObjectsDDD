...
namespace MyMapObjects
{
    ...
    public static class moMapTools
    {
        ...
        /// <param name="tolerance"></param>
        /// <returns></returns>
        public static bool IsPointOnSegment(moPoint point,moPoint node1,moPoint node2,double tolerance)
        {
            ...
            moRectangle sBox = new moRectangle(MinX - tolerance, MaxX + tolerance, MinY - tolerance, MaxY + tolerance);
            if (IsPointWithinBox(point, sBox) == false)
                return false;
            if (GetDistanceFromPointToSegment(point.X, point.Y, node1.X, node1.Y, node2.X, node2.Y) <= tolerance)
            {
                return true;
            }
            else
