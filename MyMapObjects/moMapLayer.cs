...
namespace MyMapObjects
{
    public class moMapLayer
    {
        ...
        private moGeometryTypeConstant _ShapeType = moGeometryTypeConstant.Point;  // 几何要素类型
        ...
        private bool _IsDirty = false;  // 是否被修改过（true：关闭图层时需询问是否保存）
        private moFields _AttributeFields = new moFields(); // 字段集合（即字段名称的集合）
        ...
        public moRectangle GetSeletionExtent()
        {
            if (SelectedFeatures.Count == 0)
                return Extent;
            else
            {
                moRectangle selectedExtent;
                Int32 selectedCount = SelectedFeatures.Count;
                ...
                {
                    moRectangle sExtent = _SelectedFeatures.GetItem(i).GetEnvelope();
                ...
                selectedExtent = new moRectangle(sMinX, sMaxX, sMinY, sMaxY);
        ...
        /// <param name="selecMethod"></param>

        public void ExcuteSelect(moFeatures features,Int32 selectMethod)
        {
            // 说明：此处仅考虑新建集合
            if (selectMethod == 0)
            {
        ...
        /// <param name="tolerance"></param>
        /// <returns></returns>
        public moFeatures SearchSelectedFeaturesByPoint(moPoint point, double tolerance)
        {
            moFeatures sSelectedFeatures = new moFeatures();
            Int32 sSelectedFeatureCount = _SelectedFeatures.Count;
            for (Int32 i = 0; i <= sSelectedFeatureCount - 1; i++)
            {
                if (_ShapeType == moGeometryTypeConstant.Point)
                {
                    moPoint sPoint = (moPoint)_SelectedFeatures.GetItem(i).Geometry;
                    if (moMapTools.IsPointOnPoint(point, sPoint, tolerance) == true)
                    {
                        sSelectedFeatures.Add(_SelectedFeatures.GetItem(i));
                    }
                }
                else if (_ShapeType == moGeometryTypeConstant.MultiPolyline)
                {
                    moMultiPolyline sMultiPolyline = (moMultiPolyline)_SelectedFeatures.GetItem(i).Geometry;
                ...
                {
                    moMultiPolygon sMultiPolygon = (moMultiPolygon)_SelectedFeatures.GetItem(i).Geometry;
        ...
        /// <returns></returns>
        /// 
        public moMapLayer Clone()
        {
            moFields sFields = new moFields();
            Int32 fieldCount = AttributeFields.Count;
            for (int i = 0; i <= fieldCount - 1; i++)
            {
                sFields.Append(AttributeFields.GetItem(i));
            }
            sFields.PrimaryField = AttributeFields.PrimaryField;
            sFields.ShowAlias = AttributeFields.ShowAlias;
            moMapLayer sLayer = new moMapLayer(Name, ShapeType, sFields);
            sLayer.Visible = Visible;
            sLayer.Selectable = Selectable;
            sLayer.Description = Description;
            sLayer.IsDirty = IsDirty;
            Int32 featureCount = Features.Count;
            for (int i = 0; i <= featureCount - 1; i++)
            {
                sLayer.Features.Add(Features.GetItem(i));
            }
            sLayer.UpdateExtent();
            return sLayer;
        }
        ...
        /// <param name="mpu">地图坐标单位对应的米数（一般mpu=1）</param>
        ...
        internal void DrawFeatures(Graphics g,moRectangle extent,double mapScale,double dpm,double mpu)
        {
            ...
            for (Int32 i = 0; i <= sFeaturesCount - 1; i++)
            {
        ...
        {
            // 判断是否位于绘制范围内，如是，则绘制
            Int32 sFeaturesCount = _SelectedFeatures.Count;
            for (Int32 i = 0; i <= sFeaturesCount - 1; i++)
            {
        ...
        // 绘制所有注记
        internal void DrawLabels(Graphics g, moRectangle extent, double mapScale, double dpm,
            double mpu, List<RectangleF> placedLabelExtents)
        {
            // placedLabelExtent 是已经标注的内容，新加的标注要和这里面的内容进行冲突检测，并被添加至其中
