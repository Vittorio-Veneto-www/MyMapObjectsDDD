...
namespace MyMapObjectsDemo
{
    ...
    /// </summary>
    public partial class dddLayerProperties : Form
    {
        #region 字段
        private dddMain mainFrm; //调用该窗口的主窗口
        private MyMapObjects.moMapLayer mapLayer; //该窗体所对应的图层
        private dddSource sFrmSource;
        private dddLabels sFrmLabels;
        private dddPointSymbology sFrmPointSymbology;
        private dddLineSymbology sFrmLineSymbology;
        private dddFillSymbology sFrmFillSymbology;
        ...
        #region 构造函数
        public dddLayerProperties(dddMain mainForm, MyMapObjects.moMapLayer layer)
        {
            InitializeComponent();
            mainFrm = mainForm;
            mapLayer = layer;
        ...
        {
            sFrmSource = new dddSource(mapLayer);
            ...
            sFrmLabels = new dddLabels(mapLayer);
            ...
            switch (mapLayer.ShapeType)
            {
                case MyMapObjects.moGeometryTypeConstant.Point:
                    sFrmPointSymbology = new dddPointSymbology(mapLayer);
                ...
                case MyMapObjects.moGeometryTypeConstant.MultiPolyline:
                    sFrmLineSymbology = new dddLineSymbology(mapLayer);
                ...
                case MyMapObjects.moGeometryTypeConstant.MultiPolygon:
                    sFrmFillSymbology = new dddFillSymbology(mapLayer);
        ...
        private void SaveEdit()
        {
            ...
            {
                type = form.GetType();
                if (typeof(dddSource) == type)
                {
                    dddSource sFrmSource = (dddSource)form;
                    sFrmSource.SaveEdit();
                }
                else if (typeof(dddLabels) == type)
                {
                    dddLabels sFrmLabels = (dddLabels)form;
                    sFrmLabels.SaveEdit();

                }
                else if (typeof(dddPointSymbology) == type)
                {
                    dddPointSymbology sFrmPointSymbology = (dddPointSymbology)form;
                    sFrmPointSymbology.SaveEdit();
                }
                else if (typeof(dddLineSymbology) == type)
                {
                    dddLineSymbology sFrmLineSymbology = (dddLineSymbology)form;
                    sFrmLineSymbology.SaveEdit();
                }
                else if (typeof(dddFillSymbology) == type)
                {
                    dddFillSymbology sFrmFillSymbology = (dddFillSymbology)form;
